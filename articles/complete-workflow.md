...
Same two calls, pointed at the final model:
...
ss <- add_covariates_to_search(res$search_state,
                               "data/derived/covariate_search_smk.csv")

add <- add_covariate_to_model(
  ss,
  base_model_id = res$final_model,
...
In a new session `ss` is gone - load it back from a checkpoint first.
They are written to `<models_folder>/scm_rds/` as `NN_phase_event.rds`,
zero-padded by step, so the last one alphabetically is the most recent:

``` r

list.files("models/scm_rds")
#> "00_base_complete.rds" "01_forward_step_complete.rds" ...

ss <- load_search_state("models/scm_rds/07_forward_selection_complete.rds")
ss <- update_all_model_statuses(ss)
```
