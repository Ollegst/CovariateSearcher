...
| Function | Exp | Purpose | Key inputs | Returns | Side effects | Calls / Called-by |
|---|---|---|---|---|---|---|
| `create_covariate_boxplots(data, spec, con=NULL, cat=NULL, type=c("AUC","Cmax","Cmin"), drug, param_info, stratification=NULL, total_panel=TRUE, output_folder="results/figure/simulations", width=NULL, height=NULL, base_size=10, verbose=TRUE, combined_pdf=TRUE, prefix=NULL, output_format=c("emf","png"), show_median=TRUE, show_n=TRUE, percent_change=FALSE, label_size=NULL)` | ✓ | Boxplots per metric×covariate (continuous→quartile bins, categorical→decoded), + REF box, per-cov `.emf` + combined PDF | data, `spec` (yspec), con/cat, type | nested list `results[[type]][[cov]]` = ggplot (invisible) | **file write** `.emf` via `devEMF`, `grDevices::cairo_pdf`; `dir.create`; `yspec::ys_load/ys_get_short_unit`; `grid::unit`; `stop` | standalone (not in the forest data flow) |
