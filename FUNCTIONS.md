...
- `cat.power` (valid option, e.g. dose 35/70/125/150 mg): **write side OK** — `model_add_cov` FLAG "2" correctly emits `(cov/ref)**THETA` (shared with `con.power`, benign; both should be power). **Latent bug is on the read side**: `apply_covariate_model` (apply-covariate-model.R:182) gates the categorical `IF`-parse branch on `status=="cat"` alone, so `cat.power` wrongly enters it (no `IF(COV.EQ.x)` block exists → mis-reconstructed/skipped in forest plots). One-line fix: gate on `status=="cat" && formula=="linear"`, letting `cat.power` fall to the continuous `switch` (`"power"` → `(cov_val/ref)^th`). **Deferred (owner, 2026-07-19)** → revisit with the covariate-formula registry in C.
