...
| `.transform_iteration_columns(ext_data, value_cols, model_name, models_dir, n_theta_ext)` | · | Rescale iteration columns to the scale [`model_report()`](https://ollegst.github.io/CovariateSearcher/reference/model_report.md) reports: `;LOG`-annotated THETA → [`exp()`](https://rdrr.io/r/base/Log.html), diagonal OMEGA/SIGMA → CV% `100*sqrt(exp(v)-1)`. Off-diagonals and unannotated THETAs untouched. Reach for it only from the plotter — it needs `.ext`-shaped column names | tidy `.ext` frame, `value_cols` = **every plotted column incl. `obj_var`** (so a THETA/variance promoted to the leading panel is rescaled too; `"OBJ"` matches no rule and is untouched), model name, models dir, `n_theta_ext` = count of `THETA\d+` columns in the whole `.ext` | `list(data=<rescaled frame>, labels=<named chr, name → "NAME [natural]"/"NAME [CV%]">)` | two [`warning()`](https://rdrr.io/r/base/warning.html)s (never [`stop()`](https://rdrr.io/r/base/stop.html)), both degrading to “LOG THETAs as estimated, variances still CV%’d”: control stream unreadable, or **annotation count ≠ `n_theta_ext`** — `extract_params` drops a `$THETA` line lacking its `; NAME ; UNIT ; TRANS` comment, which would shift the positional THETA alignment onto the wrong column | calls `extract_model_params` (model-output-tables.R, via `tryCatch`) → reads control stream from disk; called-by `plot_nonmem_iterations` |
| `plot_nonmem_iterations(model_name, models_dir="models", transform=TRUE, skip_iterations=0, obj_var="OBJ", max_iterations=100)` | ✓ | Faceted ggplot of the objective column + non-fixed parameter trajectories for the last non-Evaluation `$EST` step (the only step, if there is just one); BURN iterations renumbered to precede ITER | name, dir; `transform=TRUE` rescales panels (see `.transform_iteration_columns`) and suffixes facet labels; `obj_var` names the leading panel’s column; `max_iterations` is an ITERATION **ceiling**, not a count; `skip_iterations` drops that many from the start of each `$EST` | `ggplot` | [`stop()`](https://rdrr.io/r/base/stop.html) on non-scalar `model_name`, missing `.ext`, empty/iteration-free data, non-logical `transform`, or `obj_var` not a column of the `.ext` (message lists available columns); inherits `.transform_iteration_columns`’ two warnings | calls `read_ext_iterations`, `.transform_iteration_columns` (passing `plot_vars` — every plotted column — plus `n_theta_ext`, counted from the full `.ext` before constant columns are dropped so it survives `obj_var` and FIX-parameter removal), [`tidyr::pivot_longer`](https://tidyr.tidyverse.org/reference/pivot_longer.html), [`dplyr::filter`](https://dplyr.tidyverse.org/reference/filter.html), `ggplot2::` |
