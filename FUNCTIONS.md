...
**What this is:** an agent-facing map of *every* function in `R/`, so future sessions don't have to re-read the whole codebase. Originally generated from a full read of the pre-refactor code (v0.1.30). **Last audited 2026-07-30 against v0.2.0: 29 files in `R/`, 132 top-level functions, 90 exports in NAMESPACE** — every defined function appears below. Keep it in sync as the modular reorg lands.
...
| Function | Exp | Purpose | Key inputs | Returns | Side effects | Calls / Called-by |
|---|---|---|---|---|---|---|
| `load_existing_search(base_model_path, data_file_path, covariate_search_path, models_folder="models", timecol="TIME", idcol="ID", threads=60, lookup_file=NULL)` | ✓ | Discovery-mode wrapper: re-init from existing models | CSV/model paths | `search_state` or `stop()` | `cat`; `list.files`; `stop()` | calls `initialize_covariate_search` |
...
| Function | Exp | Purpose | Key inputs | Returns | Side effects | Calls / Called-by |
|---|---|---|---|---|---|---|
| `discover_existing_models(search_state)` | ✓ | Catalog `run\d+.(ctl|mod)`, read bbr per run, build `search_database`, assign relative step numbers. Every row it builds is stamped `created_by_search = FALSE` — it was on disk before the search started, so the search will not submit, retry or status-track it | `search_state` | updated `search_state` (+ `$discovered_models`) | `cat`; `list.files`; `bbr::read_model`; `stop()` (missing base / circular parent) | calls `initialize_search_database_core`, `get_model_status_from_files`, `read_nonmem_ext`, `get_model_covariates_from_files`, `dplyr::bind_rows`; nested `get_step_number`, `is_descendant_of`; called-by `initialize_covariate_search` |
...
| Function | Exp | Purpose | Key inputs | Returns | Side effects | Calls / Called-by |
|---|---|---|---|---|---|---|
| `update_model_status_from_files(search_state, model_name, force=FALSE)` | ✓ | Read `.lst`/`.ext`/`.cov`, set status/ofv/rse_max/issue, compute signed ΔOFV vs parent (only when the model is `completed` with a non-NA OFV and the parent row exists with an OFV). **The sign convention is now `.is_removal_model()`**, not an inline `action` test: removal ⇒ `child − parent` (positive = OFV rose = the removal hurt ⇒ keep the covariate), anything else ⇒ `parent − child` (positive = OFV fell = good addition). Routing it through the helper corrects the sign for a **retry of a removal** model — its own `action` is `"retry"`, so it previously fell to the forward branch and reported ΔOFV with the wrong sign in backward elimination | state, name, force | updated `search_state` | `cat`; `readLines`; **self-heals missing DB cols** (13-col `required_cols` — note `action` is *not* in it, which `.is_removal_model` tolerates by returning FALSE) | calls `extract_nonmem_timestamps`, `read_nonmem_ext`, `get_param2`, `.is_removal_model`; called-by `force_update_models`, `update_all_model_statuses` |
