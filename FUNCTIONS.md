...
audited 2026-07-31 against v0.4.2: 29 files in `R/`, 135 top-level
...
| Function | Exp | Purpose | Key inputs | Returns | Side effects | Calls / Called-by |
|----|----|----|----|----|----|----|
| `update_model_status_from_files(search_state, model_name, force=FALSE)` | ✓ | Read `.lst`/`.ext`/`.cov`, set status/ofv/rse_max/issue, compute signed ΔOFV vs parent (only when the model is `completed` with a non-NA OFV and the parent row exists with an OFV). **File resolution is delegated, not hardcoded:** the `.lst` comes from a single `read_nonmem_lst(model_path)` probe (so the run-dir *or* control-stream location is found), whose `$file` locates the listing **and** whose `$status` is checked first — a probe of `"read_error"` returns `failed` with the probe’s own `error_message`, never `in_progress`, because the monitoring loop waits on `in_progress` and a file that will never become readable would hang it forever. The `.ext` comes from a single `read_nonmem_ext(model_path)` call whose result is reused for OFV/parameters, and the `.cov` is looked for in `dirname(lst_file)` — wherever the rest of that run’s output actually is. An absent/empty/unparseable `.ext` now reports [`read_nonmem_ext()`](https://ollegst.github.io/CovariateSearcher/reference/read_nonmem_ext.md)’s own `$error` message (falling back to `"No EXT file found"` only when it is empty). **The ΔOFV sign convention is `.is_removal_model()`**, not an inline `action` test: removal ⇒ `child − parent` (positive = OFV rose = the removal hurt ⇒ keep the covariate), anything else ⇒ `parent − child` (positive = OFV fell = good addition). Routing it through the helper corrects the sign for a **retry of a removal** model — its own `action` is `"retry"`, so it previously fell to the forward branch and reported ΔOFV with the wrong sign in backward elimination | state, name, force; `$search_config$require_cov_step` (`%||% TRUE`) | updated `search_state` | `cat`; reads the listing via `.read_listing_lines` (latin1/`skipNul`, not bare `readLines` — only the bbi `OUTPUT` file is read with `readLines`); **self-heals missing DB cols** (13-col `required_cols` — note `action` is *not* in it, which `.is_removal_model` tolerates by returning FALSE); each missing column is filled with a scalar default of its own type via `if`/`else` — the previous [`dplyr::case_when()`](https://dplyr.tidyverse.org/reference/case-and-replace-when.html) there **always threw**, since its branches mixed `NA_real_`/`FALSE`/`NA_character_` and `case_when` requires one shared type, so this recovery path crashed instead of recovering | nested `read_all_model_files`; calls `read_nonmem_lst`, `.read_listing_lines`, `read_nonmem_ext`, `extract_nonmem_timestamps`, `get_param2`, `.is_removal_model`; called-by `force_update_models`, `update_all_model_statuses` |
...
| `read_nonmem_lst(model_path)` | ✓ | Classify `.lst` run status via regex (hard-failure markers beat success markers, then `Stop Time:` + runtime-error/severe-warning markers, else `incomplete`); also **the resolver for where the output lives** — 3 candidates (`<path>/<name>.lst`, `dirname(<path>)/<name>.lst`, `<path>.lst`), so callers use `$file`/`dirname($file)` to find sibling output instead of assuming a layout. Reads through `.read_listing_lines()`, so a listing with non-UTF-8 bytes still classifies | dir or `.lst` | `list(found, file, status, error_message, error_excerpt, has_issues)`; status ∈ `not_run`/`incomplete`/`completed`/`failed`/`read_error`/`invalid_input`/`invalid_path`. **`$file` is present on every path where a listing was resolved — including the read-error handler, which returns `found=TRUE`, `status="read_error"` and the path.** Dropping `$file` there used to make an unreadable listing indistinguishable from one NONMEM has not written yet, so callers testing only `$file` read it as “still running”. The three `found=FALSE` returns (`invalid_input`, `invalid_path`, `not_run`) carry no `$file` | — | calls `.read_listing_lines`; called-by `get_model_status_from_files`, `update_model_status_from_files`, `extract_nonmem_timestamps`, `validate_base_model_for_search` |
...
| Function | Exp | Purpose | Key inputs | Returns | Side effects | Calls / Called-by |
|----|----|----|----|----|----|----|
| `run_automated_scm_testing(search_state, base_model_id=NULL, scm_type=c("standard","selective"), starting_phase=c("forward","backward"), full_scm=TRUE, forward_p_value=NULL, backward_p_value=NULL, rse_threshold=NULL, require_cov_step=TRUE, auto_submit=TRUE, auto_retry=TRUE, save_checkpoints=TRUE, final_testing=TRUE)` | ✓ | Drive the whole SCM/SCM+ pipeline; dispatch standard vs selective forward + backward. Writes `forward_p_value`/`backward_p_value`/`max_rse_threshold` into `search_config` unconditionally (NULL ⇒ keep the configured value), but **`require_cov_step` is only written when the caller actually supplied it** (`if (missing(require_cov_step)) require_cov_step <- search_state$search_config$require_cov_step %||% TRUE`), so a FALSE chosen at [`initialize_covariate_search()`](https://ollegst.github.io/CovariateSearcher/reference/initialize_covariate_search.md) survives this argument’s TRUE default. Validated with `if (!is.logical(require_cov_step)) stop(...)` | `scm_type ∈ {standard,selective}`, `starting_phase ∈ {forward,backward}`, `full_scm`, p-values, `rse_threshold`, optional `require_cov_step` | big `list(search_state, status, ..., final_model, forward_results, initial_backward_results, final_backward_results, final_covariates, excluded_covariates, ..., final_summary)` | [`save_search_state()`](https://ollegst.github.io/CovariateSearcher/reference/save_search_state.md)→`scm_rds/NN_phase_event.rds`; [`stop()`](https://rdrr.io/r/base/stop.html); `<<-` in 3 error handlers; heavy `cat` | calls `pvalue_to_threshold`, `run_backward_elimination`, `run_stepwise_covariate_modeling`, `run_scm_selective_forward`, `run_univariate_step`, `submit_and_wait_for_step`, `select_best_model`, `get_model_covariates_from_db`, `get_excluded_covariates`, `save_search_state`; **top-level (no in-repo callers)** |
...
| `.is_removal_model(search_state, model_name)` | · | Direction test — is this model a covariate **removal** rather than an addition? Looks each name up in the DB and reads `action`, then resolves retry rows: `create_retry_model` writes `action="retry"`/`phase="retry"`, so a retry carries no direction of its own and only the `original_model` it points at does. TRUE when the **resolved** action matches `/remove/i`, which catches both written spellings (`remove_covariate` and `remove_single_covariate` from `model-discovery.R`); `add_covariate`/`add_single_covariate`, `manual_modification`, `base_model` and an unresolved retry are all FALSE. Exists because two places need the direction and neither can trust `action` alone: the ΔOFV sign convention and retry eligibility | state, character vector of model names | logical vector, one element per `model_name` (`logical(0)` for empty input). FALSE for a name with no DB row, an `NA` action, an action naming no direction, a dangling retry (its `original_model` has no row), a DB with no `original_model` column (retry left unresolved), and a DB with no `action` column at all | none | called-by `update_model_status_from_files` (ΔOFV sign), `submit_and_wait_for_step` (suppress retry for failed removals) |
