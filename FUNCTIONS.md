...
audited 2026-07-30 against v0.4.1: 29 files in `R/`, 132 top-level
...
return path overwrites them). **ΔOFV sign:** removal (per
`.is_removal_model`) uses `child - parent`; everything else
`parent - child` (positive=improvement both). Completion needs valid OFV
AND (if `require_cov_step`) a `.cov` **in the directory the `.lst` was
found in**; boundary params (`abs >= 8.99990e5`) → failed. This is
deliberately the same test as
...
rather than assuming a layout. Note `$file` is **absent** from the
not-found return, so it must be guarded by `$found`. `read_model_file`’s
`file_path` attr is a hard contract for `write_model_file`.
