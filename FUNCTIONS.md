...
**What this is:** an agent-facing map of *every* function in `R/`, so future sessions don't have to re-read the whole codebase. Originally generated from a full read of the pre-refactor code (v0.1.30). **Last audited 2026-07-30 against v0.4.1: 29 files in `R/`, 132 top-level functions, 90 exports in NAMESPACE** — every defined function appears below. Keep it in sync as the modular reorg lands.
...
**Notes:** **Self-heal schema #2** — `update_model_status_from_files` declares a 13-col `required_cols` subset, adds missing via `case_when` defaults (`NA_real_` for numerics, `FALSE` for `excluded_from_step`, else `NA_character_`). **Status values it writes:** `completed`/`failed`/`in_progress` only — its `results$status <- "unknown"` initial value and the internal `"stopped"`/`"read_error"` LST classifications never reach the DB (every return path overwrites them). **ΔOFV sign:** removal (per `.is_removal_model`) uses `child - parent`; everything else `parent - child` (positive=improvement both). Completion needs valid OFV AND (if `require_cov_step`) a `.cov` **in the directory the `.lst` was found in**; boundary params (`abs >= 8.99990e5`) → failed. This is deliberately the same test as `get_model_status_from_files()` — the two readers must agree, or a model changes verdict depending on which one looked.
...
| `read_nonmem_lst(model_path)` | ✓ | Classify `.lst` run status via regex; also **the resolver for where the output lives** — 3 candidates (`<path>/<name>.lst`, `dirname(<path>)/<name>.lst`, `<path>.lst`), so callers use `$file`/`dirname($file)` to find sibling output instead of assuming a layout | dir or `.lst` | `list(found, file, status, error_message, error_excerpt, has_issues)`; status ∈ `not_run`/`incomplete`/`completed`/`failed`/`read_error`/`invalid_input`/`invalid_path`, and every `found=FALSE` return carries **no `$file`** | — | called-by `get_model_status_from_files`, `update_model_status_from_files`, `extract_nonmem_timestamps`, `validate_base_model_for_search` |
| `get_model_status_from_files(model_path, require_cov_step=TRUE)` | ✓ | Combine LST+EXT into one status — **the file-side definition of "did this run succeed"**, matching `update_model_status_from_files()` so both readers agree. Order matters: *finished* is established before any OFV is trusted, because `read_nonmem_ext()` falls back to the last iteration line, so a still-estimating model reports a usable-looking OFV. How minimisation *ended* is deliberately not part of the test — a run that stopped on rounding errors but produced an OFV and a `.cov` is a success | model dir (either reader resolves run-dir vs beside-the-`.ctl`), `require_cov_step` | char status: `"not_run"` (neither `.lst` nor `.ext` found) · `"incomplete"` (no `.lst`, or the `.lst`'s own status is `"incomplete"`) · `"completed"` (OFV non-NA, finite, `abs <= 1e10`; no parameter at `abs >= 8.9999e5`; and — when `require_cov_step` — `<basename>.cov` exists in `dirname(lst_info$file)`) · `"failed"` (that `.cov` missing, **or** the `.lst` reports completion while the OFV is unusable — a missing/unparseable `.ext`, which `update_model_status_from_files()` also reports as failed) · otherwise the `.lst`'s own status (`failed`, `read_error`). **No longer returns `completed_with_issues`** — that value is gone from the package. A `"completed"` verdict therefore always means both halves of the test were met; the `.lst` alone can never produce one | — | calls `read_nonmem_lst`, `read_nonmem_ext`; called-by `validate_base_model_for_search`, `discover_existing_models` |
...
**Notes:** `grid`/`tools`/`grDevices` un-declared base-priority (`devEMF` is in `Suggests:`). `plot_exposure_forest` + `theme_forest` are exported and **in NAMESPACE**. Hardcoded palette + band strings. Gained `x_lim` (fix the ratio axis), `scenario` (merged axis order + typical-subject subtitle), `outer_range` (optional 0.5–2 band) and the summary-table arguments.
